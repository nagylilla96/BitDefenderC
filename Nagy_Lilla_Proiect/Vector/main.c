...
void printFunc(first* one, int index, FILE *f) { // prints a structure
    fprintf(f, "index = %d\n", index);
...
void printAllFunc(first* one, FILE *f) { // prints all structures of a vector
    int i;
    node **phonebook = (node **) one->pointer;
...
void freeFunct(void *a) { // frees a structure
    node *aa = (node *) a;
    free(a);
}

node *createNode(char *nume, char *prenume, char *adresa, char *telefon) { // allocate, initialize and return a node
...
node *getNode(int i, FILE *f) { // read a node and return it
...
int cmpFunct(void *a, void *b) { // compare two nodes
    node *aa = (node *) a;
    node *bb = (node *) b;
...
void freeNode(void* a){ // free a node's fields
    node *aa = (node*) a;
    free(aa->nume);
    free(aa->prenume);
    free(aa->adresa);
