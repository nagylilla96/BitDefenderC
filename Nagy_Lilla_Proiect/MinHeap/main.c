...
void *createNode(int number) { // create a new node (allocate, initialize, return)
    ELEMENT *myNode = (ELEMENT*) malloc(sizeof(ELEMENT));
    myNode->number = number;
    return myNode;
}

void *getNode(int i, FILE *f) { // scan and create a new node
    int number;
    printf("number = ");
    fscanf(f, "%d", &number);
...
void printFunc(first* one, int index, FILE *f) { // print a node's elements
    fprintf(f, "index = %d\n", index);
    ELEMENT **myNode = (ELEMENT**) one->pointer;
    fprintf(f, "number = %d\n", myNode[index]->number);
}

void anotherPrintFunc(first *one, int index, FILE *f) { // prints only one field of a node
    ELEMENT **myNode = (ELEMENT**) one->pointer;
    fprintf(f, "%d", myNode[index]->number);
}

void printAllFunc(first* one, FILE *f) { // print all the elements of the heap
    int i;
...
void freeNode(void *a) { // free a node (not needed)
    return;
}
