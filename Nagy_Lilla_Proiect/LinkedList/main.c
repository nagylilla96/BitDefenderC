...
void printFunct(void *a, FILE *f) { // prints a structure
    ELEMENT *aa = (ELEMENT*) a;
    fprintf(f, "index = %d\n", aa->index);
}

ELEMENT *createElement(int index) { // creates (allocates, initializes, returns) an element
    ELEMENT *element = malloc(sizeof(ELEMENT));
    element->index = index;
    return element;
}

void freeNode(void* a) { // free a node (we don't need this)
    return;
}
...
void swap(void *a, void *b){ // swaps two structures with each other
    NODE *aa = (NODE*) a;
    NODE *bb = (NODE*) b;
    ELEMENT *temp;
    temp = aa->element;
