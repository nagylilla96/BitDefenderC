...
int cmpFunct(void *a, void *b) { // a function that compares two elements
...
void printData(void *a, FILE *f) { // a function that prints one element's data
...
void runTester(char *nameIN, char *nameOut) { //run the tester
    BalancedTree **arrayOfTrees = calloc(26, sizeof(BalancedTree*)); // create an array of BSTs
    printf("RunTester\n");
        ...
        if (token != NULL) {
            switch (commandNr(token)) {
                case 0://creates a list to the arrayOfTrees starting from A to Z, depending on the set's name
                    ...
                    break;
                case 1: // prints BST in tree form
                    token = strtok(NULL, " ");
                    token[strlen(token) - 1] = '\0';
                ...
                case 4: // prints tree nodes postorder
                    ...
                    break;
                case 5: //adds some items an item
                    token = strtok(NULL, " ");
                    token[strlen(token) - 1] = '\0';
                    if (token != NULL && strlen(token) == 1) {
                        index = (int) *token - 65;
                ...
                case 6: // searches for an item
                    ...
                    break;
                case 7: // deletes an item from the tree
                    token = strtok(NULL, " ");
                    token[strlen(token) - 1] = '\0';
                    if (token != NULL && strlen(token) == 1){
                        index = (int) *token - 65;
                ...
                case 8: // merges two trees into a third one
                    ...
                    break;
                case 9: // returns the height of the tree
                    token = strtok(NULL, " ");
                    token[strlen(token) - 1] = '\0';
                    if (token != NULL && strlen(token) == 1){
                        index = (int) *token - 65;
                    ...
                    break;
                case 10: // deletes a tree completely
                    token = strtok(NULL, " ");
                    token[strlen(token) - 1] = '\0';
                    if (token != NULL && strlen(token) == 1 && arrayOfTrees[(int) *token - 65] != NULL){
                        index = (int) *token - 65;
